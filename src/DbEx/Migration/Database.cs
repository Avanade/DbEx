...
namespace DbEx.Migration
{
    ...
    public class Database<TConnection>(Func<TConnection> create, DbProviderFactory provider) : IDatabase where TConnection : DbConnection
    {
        ...
        /// <inheritdoc/>
        public DatabaseCommand StoredProcedure(string storedProcedure) => new(this, CommandType.StoredProcedure, storedProcedure.ThrowIfNull(nameof(storedProcedure)));

        /// <inheritdoc/>
        public DatabaseCommand SqlStatement(string sqlStatement) => new(this, CommandType.Text, sqlStatement.ThrowIfNull(nameof(sqlStatement)));
