    ...
    public abstract class EventOutboxDequeueBase : IDatabaseMapper<EventSendData>
    {
        ...
        public async Task<int> DequeueAndSendAsync(int maxDequeueSize = 50, string? partitionKey = null, string? destination = null, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                var events = await DequeueAsync(maxDequeueSize, partitionKey, destination).ConfigureAwait(false);
                sw.Stop();
        ...
        /// </summary>
        private async Task<IEnumerable<EventSendData>> DequeueAsync(int maxDequeueSize, string? partitionKey, string? destination) 
            => await Database.StoredProcedure(DequeueStoredProcedure, p => p.Param("@MaxDequeueSize", maxDequeueSize).Param("@PartitionKey", partitionKey).Param("@Destination", destination))
                             .SelectAsync(this).ConfigureAwait(false);

        /// <inheritdoc/>
        public EventSendData MapFromDb(DatabaseRecord record)
        {
